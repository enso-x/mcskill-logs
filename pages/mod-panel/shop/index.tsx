...
import { EStoneType, STONES } from '@/data/mod-panel/stones';
...
import {
	...
	siteShopItemFilter,
	stoneItemFilter
} from '@/data/mod-panel/shop/filters';
	...
	const tabItems = useMemo(() => {
		return categories ? categories.map(shopCategory => shopCategory.id === 'groups' ? ({
			...
			children: (
				<ItemsContainer>
					{
						siteGroupItemFilter(filterString)(groups).map(shopGroup => (
			...
			children: (
				<ItemsContainer>
					{
						baseShopItemFilter(filterString)(SERVICES).map(item => (
			...
			children: (
				<VerticalLayout>
					<Tabs items={[
						{
							key: `${ EStoneType.Functional }`,
							label: 'Функциональные',
							children: (
								<ItemsContainer>
									{
										stoneItemFilter(EStoneType.Functional)(filterString)(STONES).map(item => (
											<ShopItem key={ item.id } user={ user }
						...
						},
						{
							...
							children: (
								<ItemsContainer>
									{
										stoneItemFilter(EStoneType.Form)(filterString)(STONES).map(item => (
											<ShopItem key={ item.id } user={ user }
											          item={ item } type={ EOrderType.item }
											          selectedServer={ selectedServer } buttonsDisabled={ buttonsDisabled }
											          beforeSubmit={ beforeSubmitBuy } onSubmit={ onSubmitBuy }/>
			...
			children: (
				<ItemsContainer>
					{
						siteShopItemFilter(shopCategory.id)(filterString)(items).map(siteShopItemAdapter).map(item => (
							<ShopItem key={ item.id } user={ user }
							          item={ item } type={ EOrderType.item }
							          selectedServer={ selectedServer } buttonsDisabled={ buttonsDisabled }
							          beforeSubmit={ beforeSubmitBuy } onSubmit={ onSubmitBuy }/>
